...
// insert an Edge
// - sets (v,w) and (w,v)
void insertEdgeEdge (Graph g, Vertex v, Vertex w, int wt)
{Edge
	assert (g !Edge= NULL && validV (g, v) && validV (g, w));
Edge
	if (g->edgeEdges[v][w] != 0 && g->edges[w][v] != 0)
		// an eEdgedge already exists; do nothing.
		return;Edge
