...
static int addType(TransportID type[], int nTransport, TransportID id){
	int haveType = 0;
	...
	if(!haveType) {type[nTransport]=id;return 1;}
	else return 0;
}
...
int connections (Map g, LocationID start, LocationID end, TransportID type[])
{
	...
	// find direct path and if it connect to the sea
	LocationID  seaName = NOWHERE;
	for(VList node = g->connections[start] ; node!=NULL ; node = node->next){
		if(node->v == end){
			if(addType(type,nTransport,node->type))
				nTransport++;
		}else if(node->type == BOAT){
			seaName = node->v;
		}
	}

	// if it connects to the sea, search if another place connects to the sea 
	if(seaName != NOWHERE){
		for(VList node = g->connections[seaName] ; node!=NULL ; node = node->next){
			if(node->v == end){
					if(addType(type,nTransport,node->type))
						nTransport++;
			}
